...
class Notification < Sinatra::Base
  ...
  get '/notification/install' do
    ...
    install_url = "http://#{shop}/admin/oauth/authorize?client_id=#{API_KEY}"\
                "&scope=#{scopes}&redirect_uri=https://#{APP_URL}/notification/auth"

    # redirect to the install_url
    redirect install_url
  end

  get '/notification/auth' do
  ...
  post '/notification/webhook/order_create' do
  ...
  helpers do
    ...
    def create_order_webhook
      ...
      unless ShopifyAPI::Webhook.find(:all).any?
        webhook = {
          topic: 'orders/create',
          address: "https://#{APP_URL}/notification/webhook/order_create",
          format: 'json'}
